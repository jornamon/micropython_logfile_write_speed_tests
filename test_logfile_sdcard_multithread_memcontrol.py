...
# Attempt to manage queue size by sampling loop sleeping more if queue is too big
...
def core1(q_data):
    ...
    for row in range(ROWS):
        ...
        time.sleep_ms(SENSOR_DELAY)
                ...
                # Avoid queue to grow until memory runout
#         if gc.mem_free() < MEM_MIN:
#             #print("min mem thriggered")
#             time.sleep_ms(MEM_SLEEP)
        if q_data.qsize() > MAX_QUEUE:
            while q_data.qsize() > MIN_QUEUE:
                time.sleep_ms(MEM_SLEEP)
            #gc.collect()
